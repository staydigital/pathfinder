...
const pathfinderReducer = createReducer(
  ...
  on(finishCell, (state, { id }) => {
    return onFinishCell(state, id)
  }),
  ...
  on(setAStarCost, (state, { id, gcost, hcost }) => {
    return onSetAStarCost(state, id, gcost, hcost)
  }),
  on(setAStarPrecessor, (state, { id, precessor }) => {
    return onSetAStarPrecessor(state, id, precessor)
  })
)
...
function onFinishCell (state: State, id: string) {
  const cellUpdate: Update<CellState> = {
    id: id,
    changes: { mode: CellMode.FINISH }
  }
  return cellStateEntityAdapter.updateOne(cellUpdate, state)
}
...
function onSetAStarCost (
  ...
  const cellUpdate: Update<CellState> = {
    id: id,
    changes: { gcost: gcost, hcost: hcost, totalCost: gcost + hcost }
  }
  return cellStateEntityAdapter.updateOne(cellUpdate, state)
}

function onSetAStarPrecessor (state: State, id: string, precessor: string) {
  const cellUpdate: Update<CellState> = {
    id: id,
    changes: { precessor: precessor }
  }
  return cellStateEntityAdapter.updateOne(cellUpdate, state)
}
